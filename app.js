...
function getAdjacentBeastDirections() {
  const order = ['forward', 'right', 'left', 'backward'];
  return order.filter((direction) => isAdjacentBeast(direction));
}

function getNearbyBeastImage(adjacentDirections) {
  if (!adjacentDirections.length) {
    return null;
  }

  const seed = `${adjacentDirections.join('|')}|${game?.turn ?? 0}`;
  let total = 0;
  ...
  return BEAST_NEARBY_IMAGES[total % BEAST_NEARBY_IMAGES.length];
}
...
function updateSceneImage() {
  ...
  const adjacentDirections = getAdjacentBeastDirections();
  if (adjacentDirections.length > 0) {
    elements.sceneImage.src = getNearbyBeastImage(adjacentDirections);
    elements.sceneImage.alt = adjacentDirections.length > 1
      ? 'More than one Beast is right next to the player'
      : 'A Beast is right next to the player';
    elements.sceneImageLabel.textContent = adjacentDirections.length > 1
      ? 'Beasts are right next to you. Try Catch.'
      : 'A Beast is right next to you. Try Catch.';
    return;
  }
