...
function getShortestPath(size, start, goal, wallSet) {
  ...
  for (let index = 0; index < queue.length; index += 1) {
    ...
    for (const neighbor of getNeighbors(size, current.x, current.y, wallSet)) {
      ...
      parents.set(neighborKey, toKey(current.x, current.y));
      ...
      if (neighborKey === goalKey) {
        ...
        while (cursorKey !== startKey) {
          const parentKey = parents.get(cursorKey);
          if (!parentKey) {
            break;
          }
          path.push(fromKey(parentKey));
          cursorKey = parentKey;
        }
...
function getNextBeastPosition(game) {
  const wallSet = createWallSet(game.walls);
  const path = getShortestPath(game.size, game.beast, game.player, wallSet);
